import * as formatters from "../src/formatters"; //relative path to the file
/*the above line imports everthing from 'formatters.ts' file 
    and used in 'higher order functions*/
describe('functions', () => {
    describe('function literals', () => {
        it('has a couple of kinds', () => {
            ...
            const multiply = (a: number, b: number) => a * b; //another way to do line 10 & 11
            expect(multiply(3, 3)).toBe(9);

            const divide = (a: number, b: number) => {
                if (b <= 0) {
                    throw new Error('You almost destoyed the universe');
                } else {
                    return a / b
                }
            }
            const age = 21;
            const message = age >= 21 ? "old Enough" : "Too Young"; //can also be done in C#
        });
    ...
    describe('array methods', () => {
        ...
        describe('methods that return new arrays', () => {
            ...
            it('do a practice', () => {
                ...
                // Give me the make and model of each vehicle with < 100_000 miles
                const lowMileageVehicles = vehicles
                    .filter(c => c.mileage < 100000)
                    .map(c => c.makeAndModel);
