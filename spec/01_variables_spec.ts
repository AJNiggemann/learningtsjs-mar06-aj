describe('variables and constants', () => {
    describe('declaring variables', () => {
        it('has a let keyword', () => {
            ...
            let z: number | string | Food; //Union type (gives you the common types between all three)
            //z. ;
            expect(x).toBe(12);
            x = 13;
            expect(x).toBe(13);
    ...
    describe('literals', () => {
        it('has numeric literals', () => {
            let first = 10;
            let second = 3.12;
            let salary = 10_001_800; //underscore takes the place of commas in currency
            let hexNumber = 0xff; //used in code to represent colors
            let binary = 0b101010;
            let octal = 0o744; //number needs to be a 'base 8'
        });
        it('has string literals', () => {
            ...
            let author = "Flannery O'Connel";

            expect("hi").toBe(`hi`);

            let lifeStory = `Sometimes,
            
            Something happens.
            And it Fails.`

            let name = 'Jeff', age = 49;
            let info = `His name is ${name} and his age is ${age}.`; //this ONLY works with `` (back ticks)
            console.log(info);
        });
        it('has array literals', () => {
            ...
            const friends: (string | number)[] = []; //line 75 & 76 are equivalent
            friends[0] = 'David';
            friends[1] = 'Amber';
            friends[2] = 42; //would no work is line 76 was 'const friends: string[] = [];'
            luckyNumbers[0] = 89;
        });
        describe('tuples', () => {
            it('making the case', () => {
            ...
            it('using a tuple', () => {
                ...
                // let formatName: [string, string, number];
                // formatName = ['Han', 'Solo', length];
                // let [first, last, count] = formatName;
                // expect(first).toBe('Han');
            ...
            it('using destructuring on an array', () => {
                ...
                const [firstFriend, ...restOfMyFiends] = friends;
                expect(firstFriend).toBe('Reggie');
                expect(restOfMyFiends).toEqual(['Susan', 'Neil']);
            });
    ...
    describe('objects and destructuring of objects', () => {
        ...
        it('duck typing', () => {
            ...
            const pc2: PhoneCallType = {
                message: 'Boys are driving me nuts!',
                from: 'Amber'
            }
            const { from, message: msg } = pc2;
            expect(from).toBe('Amber');
            expect(msg).toBe('Boys are driving me nuts!');
        });
    });
});